...
// ============================================================
// 搜索服务 - 多源搜索（SearXNG + DuckDuckGo fallback）
// ============================================================
  ...
  for (const instance of SEARXNG_INSTANCES) {
    try {
      ...
      if (options?.timeRange) {
        params.time_range = options.timeRange === "1d" ? "day" : options.timeRange === "1w" ? "week" : "";
      }
...
export async function searchAI(
  ...
  // 优先 SearXNG
  const searxngResults = await searchSearXNG(query, options);
  if (searxngResults.length > 0) return searxngResults;

  console.warn("[Search] SearXNG returned 0 for query:", query);
  return [];
}
