...
export async function createGenerationLog(
  type: "daily" | "weekly" | "collect" | "daily-sync" | "rss-collect" | "juya-check" | "leaderboard",
  targetDate: string
): Promise<string> {
  const client = getClient();
  const id = crypto.randomUUID();
...
export async function getNewsByDateRange(
  ...
  const end = `${endDate}T23:59:59+08:00`;
  ...
  if (error) throw new Error(`按日期范围获取新闻失败: ${error.message}`);
  return data || [];
}
