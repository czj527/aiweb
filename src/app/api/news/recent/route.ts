...
export async function GET(request: NextRequest) {
  try {
    ...
    const byDate = new Map<string, Map<string, Array<{
      id: string;
      title: string;
      source: string;
      sourceUrl: string;
    ...
    for (let i = 0; i < validDays; i++) {
      ...
      daysData.push({ date: dateStr, dateLabel, categories, totalCount: categories.reduce((sum, c) => sum + c.count, 0) });
    }

    return NextResponse.json({
      success: true,
      data: { days: daysData, fetchedAt: new Date().toISOString() },
    }, {
